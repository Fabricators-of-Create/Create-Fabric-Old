...
public class ZapperRenderHandler {
	...
	public static Vec3d getExactBarrelPos(boolean mainHand) {
		...
		boolean rightHand = mainHand == (player.getMainArm() == Arm.RIGHT);
		float zOffset = ((float) MinecraftClient.getInstance().options.fov - 70) / -100;
		Vec3d barrelPosNoTransform = new Vec3d(rightHand ? -.35f : .35f, -0.115f, .75f + zOffset);
		Vec3d barrelPos = player.getCameraPosVec(partialTicks)
			.add(barrelPosNoTransform.rotateX(pitch)
				.rotateY(yaw));
		return barrelPos;
	}
	...
	public static void playSound(Hand hand, BlockPos position) {
		...
		MinecraftClient.getInstance().world.playSound(position, AllSoundEvents.BLOCKZAPPER_PLACE.get(), SoundCategory.BLOCKS,
			0.8f, pitch, false);
	}
	...
	public static void onRenderPlayerHand(RenderHandEvent event) {
		...
		firstPersonRenderer.renderItem(mc.player, heldItem,
			rightHand ? ModelTransformation.Mode.FIRST_PERSON_RIGHT_HAND
	...
	public static void dontAnimateItem(Hand hand) {
		boolean rightHand = hand == Hand.MAIN_HAND ^ MinecraftClient.getInstance().player.getMainArm() == Arm.LEFT;
