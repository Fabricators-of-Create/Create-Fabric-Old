...
public abstract class Contraption {
	...
	protected void addBlock(BlockPos pos, Pair<StructureBlockInfo, BlockEntity> pair) {
		...
		StructureBlockInfo blockInfo = new StructureBlockInfo(localPos, captured.state, captured.tag);
	...
	protected CompoundTag getTileEntityNBT(World world, BlockPos pos) {
		...
		if (tileentity == null)
			return null;
		CompoundTag nbt = tileentity.toTag(new CompoundTag());
		nbt.remove("x");
		nbt.remove("y");
		nbt.remove("z");
	...
	private CompoundTag writeBlocksCompound() {
		CompoundTag compound = new CompoundTag();
		BiMapPalette<BlockState> palette = new BiMapPalette<>(GameData.getBlockStateIDMap(), 16, (i, s) -> {throw new IllegalStateException("Palette Map index exceeded maximum");}, NbtHelper::toBlockState, NbtHelper::fromBlockState);
	...
	private void readBlocksCompound(Tag compound, World world, boolean usePalettedDeserialization) {
		...
		if (usePalettedDeserialization) {
			...
			palette = new BiMapPalette<>(GameData.getBlockStateIDMap(), 16, (i, s) -> {throw new IllegalStateException("Palette Map index exceeded maximum");}, NbtHelper::toBlockState, NbtHelper::fromBlockState);
		...
		blockList.forEach(e -> {
			...
			if (world.isClient) {
				...
				BlockEntity te = BlockEntity.createFromTag(info.state, tag);
				if (te == null)
					return;
	...
	private static StructureBlockInfo readBlockInfo(CompoundTag blockListEntry, BiMapPalette<BlockState> palette) {
	...
	public void removeBlocksFromWorld(World world, BlockPos offset) {
		...
		for (StructureBlockInfo block : blocks.values()) {
			...
			if (!shouldUpdateAfterMovement(block))
				continue;
			world.markAndNotifyBlock(add, world.getWorldChunk(add), block.state, Blocks.AIR.getDefaultState(), BlockFlags.IS_MOVING | BlockFlags.DEFAULT, 512);
		}
	}
	...
	public void addBlocksToWorld(World world, StructureTransform transform) {
		for (boolean nonBrittles : Iterate.trueAndFalse) {
			for (StructureBlockInfo block : blocks.values()) {
				...
				if (state.getBlock() instanceof Waterloggable && BlockHelper.hasBlockStateProperty(state, Properties.WATERLOGGED)) {
