...
public class BeltRenderer extends SafeTileEntityRenderer<BeltTileEntity> {
	...
	protected void renderItems(BeltTileEntity te, float partialTicks, MatrixStack ms, VertexConsumerProvider buffer,
		...
		for (TransportedItemStack transported : te.getInventory()
			...
			boolean blockItem = itemRenderer.getHeldItemModel(transported.stack, te.getWorld(), null)
				.hasDepth();
			...
			if (MinecraftClient.getInstance().options.graphicsMode == GraphicsMode.FANCY) {
				Vec3d shadowPos = Vec3d.of(te.getPos()).add(beltStartOffset.multiply(1)
					.add(offsetVec)
			...
			for (int i = 0; i <= count; i++) {
				...
				ms.scale(.5f, .5f, .5f);
				itemRenderer.renderItem(transported.stack, Mode.FIXED, stackLight, overlay, ms, buffer);
				ms.pop();
