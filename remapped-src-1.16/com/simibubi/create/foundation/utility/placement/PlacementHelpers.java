...
public class PlacementHelpers {
	...
	private static void textured(float centerX, float centerY, float alpha, float snappedAngle) {
		...
		buffer.vertex(-1, -1, 0).color(1f, 1f, 1f, alpha).texture(tx, ty).next();
		buffer.vertex(-1, 1, 0).color(1f, 1f, 1f, alpha).texture(tx, ty + th).next();
		buffer.vertex(1, 1, 0).color(1f, 1f, 1f, alpha).texture(tx + tw, ty + th).next();
		buffer.vertex(1, -1, 0).color(1f, 1f, 1f, alpha).texture(tx + tw, ty).next();
