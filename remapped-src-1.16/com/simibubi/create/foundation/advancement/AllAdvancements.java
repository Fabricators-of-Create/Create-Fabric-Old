...
public class AllAdvancements implements DataProvider {
	...
	public void register(Consumer<Advancement> t) {
		...
		Advancement aesthetics =
			advancement("aesthetics", AllBlocks.WOODEN_BRACKET.get(), TaskType.NORMAL).parent(andesite_alloy)
				.criterion("0", AllTriggers.BRACKET_APPLY_TRIGGER.forEntries(AllBlocks.SHAFT.get()))
				.criterion("1", AllTriggers.BRACKET_APPLY_TRIGGER.forEntries(AllBlocks.COGWHEEL.get(), AllBlocks.LARGE_COGWHEEL.get()))
				.criterion("2", AllTriggers.BRACKET_APPLY_TRIGGER.forEntries(AllBlocks.FLUID_PIPE.get()))
		...
		Advancement fan = advancement("fan", AllBlocks.ENCASED_FAN.get(), TaskType.NORMAL).parent(press)
			.criterion("0", AllTriggers.FAN_PROCESSING.forEntries(InWorldProcessing.Type.NONE))
			.build(t, id + ":fan");

		Advancement fan_lava = advancement("fan_lava", Items.LAVA_BUCKET, TaskType.NORMAL).parent(fan)
			.criterion("0", AllTriggers.FAN_PROCESSING.forEntries(InWorldProcessing.Type.BLASTING))
			.build(t, id + ":fan_lava");

		Advancement fan_smoke = advancement("fan_smoke", Items.CAMPFIRE, TaskType.NORMAL).parent(fan)
			.criterion("0", AllTriggers.FAN_PROCESSING.forEntries(InWorldProcessing.Type.SMOKING))
			.build(t, id + ":fan_smoke");

		Advancement fan_water = advancement("fan_water", Items.WATER_BUCKET, TaskType.NORMAL).parent(fan)
			.criterion("0", AllTriggers.FAN_PROCESSING.forEntries(InWorldProcessing.Type.SPLASHING))
		...
		Advancement mixer = advancement("mixer", AllBlocks.MECHANICAL_MIXER.get(), TaskType.MILESTONE)
			...
			.criterion("1", AllTriggers.MIXER_MIX.instance())
			.parent(basin)
			.build(t, id + ":mixer");
	...
	void copperAge(Consumer<Advancement> t, Advancement root) {
		...
		Advancement infinite_water =
			advancement("infinite_water", Items.WATER_BUCKET, TaskType.NORMAL).parent(hose_pulley)
				.criterion("0", isInfinite(Fluids.WATER))
		...
		Advancement infinite_lava =
			advancement("infinite_lava", Items.LAVA_BUCKET, TaskType.GOAL).parent(hose_pulley)
				.criterion("0", isInfinite(Fluids.LAVA))
		...
		Advancement infinite_chocolate = advancement("infinite_chocolate", AllFluids.CHOCOLATE.get()
			.getBucketItem(), TaskType.CHALLENGE).parent(hose_pulley)
				.criterion("0", isInfinite(AllFluids.CHOCOLATE.get()))
	...
	public RegistryTrigger.Instance<Fluid> isInfinite(FlowableFluid fluid) {
		return AllTriggers.INFINITE_FLUID.forEntries(fluid.getStill());
	}
