...
public class VecHelper {
	...
	public static boolean isVecPointingTowards(Vec3d vec, Direction direction) {
		return new Vec3d(direction.getUnitVector()).distanceTo(vec.normalize()) < .75;
	}

	public static Vec3d getCenterOf(Vec3i pos) {
		if (pos.equals(Vec3i.ZERO)) // TODO ZERO INSTEAD OF NULL_VECTOR ?
			return CENTER_OF_ORIGIN;
		return new Vec3d(pos.getX(), pos.getY(), pos.getZ()).add(.5f, .5f, .5f);
	}
	...
	public static Vec3d axisAlingedPlaneOf(Direction face) {
		return axisAlingedPlaneOf(new Vec3d(face.getUnitVector()));
	}
	...
	/**public static Vec3d projectToPlayerView(Vec3d target, float partialTicks) {
		* The (centered) location on the screen of the given 3d point in the world.
		...
		Vec3f result3f = new Vec3f((float) (camera_pos.x - target.x),
		...
		if (mc.options.bobView) {
			...
			if (renderViewEntity instanceof PlayerEntity) {
				...
				float f2 = MathHelper.lerp(partialTicks, playerentity.prevStrideDistance, playerentity.strideDistance); // TODO strideDistance INSTEAD OF cameraYaw ?
				Quaternion q2 = new Quaternion(Vec3f.POSITIVE_X, Math.abs(MathHelper.cos(f1 * (float) Math.PI - 0.2F) * f2) * 5.0F, true);
				q2.conjugate();
				result3f.rotate(q2);

				Quaternion q1 = new Quaternion(Vec3f.POSITIVE_Z, MathHelper.sin(f1 * (float) Math.PI) * f2 * 3.0F, true);
				q1.conjugate();
				result3f.rotate(q1);

				Vec3f bob_translation = new Vec3f((MathHelper.sin(f1 * (float) Math.PI) * f2 * 0.5F), (-Math.abs(MathHelper.cos(f1 * (float) Math.PI) * f2)), 0.0f);
				bob_translation.set(bob_translation.getX(), -bob_translation.getY(), bob_translation.getZ());  // this is weird but hey, if it works
