...
import com.smellypengu.createfabric.content.contraptions.base.DirectionalAxisKineticBlock;
...
public class StructureTransform {
	...
	public BlockState apply(BlockState state) {
		...
		if (AllBlocks.BELT.hasTileEntity(state)) { // TODO MIGHT BE WRONG ORIGINAL supports()
			state = transformBelt(state, halfTurn);
			return state;
		}

		if (state.get(FACE) != null) {
			...
			if (state.get(HORIZONTAL_FACING) != null) {
			...
			if (state.get(SlabBlock.TYPE) != null && state.get(SlabBlock.TYPE) != SlabType.DOUBLE)
