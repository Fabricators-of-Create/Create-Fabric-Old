	...
	public static void tick() {
		...
		for (Hand hand : Hand.values()) {
			...
			if (rayTrace == null || !(rayTrace instanceof BlockHitResult)) {
				if (r.nextInt(50) == 0) {
					world.addParticle(new DustParticleEffect(new Vec3f(.3f, .9f, .5f), 1),
			...
			for (float f = 0; f < length; f += .0625f) {
				...
				if (r.nextInt(10) == 0) {
					world.addParticle(new DustParticleEffect(new Vec3f(canConnect ? .3f : .9f, canConnect ? .9f : .3f, .5f), 1),
